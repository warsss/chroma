...
float3 ChromakeyPS(float4 vois : SV_Position, float2 texcoord : TexCoord) : SV_Target
{
    ...
    // Check if the pixel belongs to a human character
    float4 depthValue = ReShade::Texture2D(ReShade::BackBufferDepth, texcoord);
    float depth = ReShade::LinearizeDepth(depthValue).x;
    float4 normal = ReShade::Texture2D(ReShade::BackBufferNormal, texcoord);
    bool isHuman = IsHuman(normal.rgb);

    // If the pixel does not belong to a human character, apply chromakey
    if (!isHuman) {
        return lerp(tex2D(ReShade::BackBuffer, texcoord).rgb, Screen, DepthMask);
    }

    // If the pixel belongs to a human character, return the original color
    return tex2D(ReShade::BackBuffer, texcoord).rgb;
}

bool IsHuman(float3 normal) {
    // You need to implement a function here that checks if the normal indicates a human character
    // For example, you can check if the normal is pointing upwards (indicating a standing character)
    // and if the normal is within a certain range of human skin tones
    // This is a simplified example and may not work perfectly for all cases
    float3 skinTone = float3(0.9, 0.78, 0.69);
    float dotProduct = dot(normal, skinTone);
